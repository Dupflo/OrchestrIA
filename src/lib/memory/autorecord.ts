...
function appendEntry(file: string, missionId: string, header: string, userText: string, agentText: string): void {
  rotateIfTooBig(file);
...
export function startAutoMemoryRecording(): boolean {
  ...
  const unsub = registry.onMissionComplete((info) => {
    try {
      ...
      const file = memoryFileFor(info.agentName, scope === "GLOBAL" ? "GLOBAL" : "USER");
      appendEntry(file, info.missionId, header, userText, agentText);
    } catch (e) {
